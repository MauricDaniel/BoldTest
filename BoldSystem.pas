  ...
  if BoldPersistent then
    ...
    begin
      if SafeGetBoldMemberAssigned(i) then
        ...
        MemberModified := false;
//                          GetElementFlag(befObjectWasCreatedNew) and
...
begin
  case CompareType of // handle explicit case
    ctCaseInsensitive:
      Result := UnicodeCompareText(false, s1, s2);
    ctCaseSensitive:
      Result := UnicodeCompareText(true, s1, s2);
    ctDefault: // handle implicit case
      case BoldDefaultStringCompareMode of
        ctCaseInsensitive:
          Result := UnicodeCompareText(false, s1, s2);
        ctCaseSensitive:
          Result := UnicodeCompareText(true, s1, s2)
...
begin
  ...
  begin
    ...
    if Assigned(NewLocator) and Assigned(NewLocator.BoldObject) and (NewLocator.BoldObject.BoldExistenceState = besDeleted) then
      raise EBold.CreateFmt('SetLocator: Cannot set to deleted object %s, expected: %s ', [NewLocator.BoldObject.DebugInfo]);
//      BoldRaiseLastFailure(self, 'SetLocator', 'Cannot set to deleted object');
