...
{------------------------------------------------------------------------------}
  ...
  protected
    ...
    procedure ValueFromField(const AField: IBoldField; const AValue: IBoldValue);
  end;
  ...
  public
    constructor Create(PersistenceController: TBoldExternalPersistenceControllerSQL;
      ExternalKeys: TBoldObjectIdList; const ValueSpace: IBoldValueSpace;
...
begin
  ...
  try
    ...
    DatabaseAdapter.DatabaseInterface.ReleaseQuery(BoldQuery);
  end;
end;
...
begin
  ...
  if Source.FieldByName(DBFieldName).IsNull then
    ...
    ValueFromField(Source.FieldByName(DBFieldName), Value);
end;

procedure TMemberFetchObject.ValueFromField(const AField: IBoldField;
  const AValue: IBoldValue);
begin
  case AValue.ContentType of
    bctValueSpace: raise Exception.CreateFmt('Unsupported type: %s', ['bctValueSpace']);
    bctObject: raise Exception.CreateFmt('Unsupported type: %s', ['bctObject']);
    bctString:        (AValue as IBoldStringContent).asString := AField.AsString;
    bctAnsiString:    (AValue as IBoldAnsiStringContent).asAnsiString := AField.AsAnsiString;
    bctUnicodeString: (AValue as IBoldUnicodeStringContent).asString := AField.AsWideString;
    bctCurrency:      (AValue as IBoldCurrencyContent).asCurrency := AField.AsCurrency;
    bctFloat:         (AValue as IBoldFloatContent).asFloat := AField.AsFloat;
    bctInteger:       (AValue as IBoldIntegerContent).asInteger := AField.AsInteger;
    bctBoolean:       (AValue as IBoldBooleanContent).asBoolean := AField.AsBoolean;
    bctDate:          (AValue as IBoldDateContent).asDate := AField.AsDate;
    bctTime:          (AValue as IBoldTimeContent).asTime := AField.AsTime;
    bctDateTime:      (AValue as IBoldDateTimeContent).asDateTime := AField.AsDateTime;
    bctBlob:          (AValue as IBoldBlobContent).asBytes := AField.AsBytes;
    bctTypedBlob: raise Exception.CreateFmt('Unsupported type: %s', ['bctTypedBlob']);
  else raise Exception.Create(sUnknownDataType);
  end;
end;
...
constructor TFetchContext.Create(
  ...
  ExternalKeys: TBoldObjectIdList;
  const ValueSpace: IBoldValueSpace; MoldClass: TMoldClass;
  MemberIdList: TBoldMemberIdList);
...
begin
  ...
  fSource := PersistenceController.DatabaseAdapter.DatabaseInterface.GetQuery;
  fSource.AssignSQLText(SQL);
  fSource.Open;
end;
...
begin
  ...
  fPersistenceController.DatabaseAdapter.DatabaseInterface.ReleaseQuery(fSource);
  inherited;
end;
...
begin
  ...
  begin
    if FetchObjectList.PersistenceController.LocateInDB(
         FetchObjectList.MoldClass, ObjectContents, fSource) then
      FetchObjectList[i].Fetch(fSource, ObjectContents)
    else
