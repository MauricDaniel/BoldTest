...
begin
  ...
  begin
    if s.IsNull then
      ...
      StringRepresentation[brMLString] := s.asString;
  end
  else
    raise EBold.CreateFmt(sUnknownTypeOfSource, [classname, 'AssignContentValue']); // do not localize
end;

function TBAMLString_Proxy.GetAsBytes: TArray<Byte>;
begin
  result := TEncoding.Default.GetBytes(ProxedMLString.AsString);
end;
...
procedure TBAMLString_Proxy.SetAsBytes(const Value: TArray<Byte>);
begin
  ProxedMLString.AsString := TEncoding.Default.GetString(Value);
end;
