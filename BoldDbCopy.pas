...
begin
  ...
  try
    repeat
      ...
      SourceQuery.Open;
      Assert((SourceQuery.AsDataSet as TUniQuery).SpecificOptions.Values['FetchAll'] = 'false');
      Assert((SourceQuery.AsDataSet as TUniQuery).SpecificOptions.Values['SQL Server.FetchAll'] = 'false');
      Assert((SourceQuery.AsDataSet as TUniQuery).FetchingAll = false);
      var i,j: integer;
      sl.Clear;
      sl2.Clear;
      j := SourceRecordCount;
...
//  var q := fAllTables.IndexOfName('CustomsTask');
//    fAllTables.Move(q, 0);
//  fTableQueue.Enqueue(fAllTables.Names[0]);
