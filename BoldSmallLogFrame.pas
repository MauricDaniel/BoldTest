...
type
  ...
  private
    ...
    procedure Clear;
    procedure Hide;
    procedure Log(const AMsg: string; LogType: TBoldLogType = ltInfo); overload;
    procedure Log(const AMsg: string; const ATags: array of const); overload;
    procedure Log(const ATags: array of const); overload;
...
procedure TBoldLogFrame.Log(const AMsg: string; const ATags: array of const);
begin
  Log(AMsg, ltInfo);
end;

procedure TBoldLogFrame.Log(const ATags: array of const);
begin
  if Length(ATags)>0 then
    Log(String(Atags[0].VUnicodeString), ltInfo);
end;

procedure TBoldLogFrame.Log(const AMsg: string);
begin
  Log(AMsg, ltInfo);
end;

procedure TBoldLogFrame.Log(const AMsg: string; LogType: TBoldLogType = ltInfo);
begin
  if LogType in [ltInfo, ltWarning, ltError] then
  begin
    lblLogText.caption := trim(AMsg);
    DoRefresh;
  end;
