...
var
  ObjectIds: TBoldObjectIdList;
  ModifiedMembers: TBoldMemberList; // Moved up to be available throughout
  oid: TBoldObjectId;
  FSObjectContents: TBoldFreeStandingObjectContents;
  ...
  var
    ...
    NilIdValue, SavIdValue: TBFSObjectIdRef;
    GInner: IBoldGuard;
  begin
    GInner := TBoldGuard.Create(NilIdValue, SavIdValue);
    GetInnerLinkIndices(BoldObject, MemberIndex0, MemberIndex1);
    if (MemberIndex0 = -1) or (MemberIndex1 = -1) then Exit; // Not a link object
  ...
  var
    oid: TBoldObjectId;
    FSObjectContents, FSRedoObjectContents: TBoldFreeStandingObjectContents;
    i, M: Integer;
    Value: IBoldValue;
    BoldObject: TBoldObject;
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          RedoValueSpace.GetFSObjectContentsByObjectId(oid).ApplyObjectContents(
            BoldObject.AsIBoldObjectContents[bdepContents], False, False);
      ...
      case FSObjectContents.BoldExistenceState of
        besExisting:
          case FSObjectContents.BoldPersistenceState of
            bvpsCurrent: Result := oaExisingPersistent;
            bvpsModified: Result := oaNewPersistent;
            bvpsTransient: Result := oaTransient;
            else raise EBold.Create(sInternalError);
          end;
        besNotCreated, besDeleted:
          case FSObjectContents.BoldPersistenceState of
            bvpsCurrent, bvpsModified, bvpsTransient: Result := oaDelete;
            else raise EBold.Create(sInternalError);
          end;
      end;
    end;
...
begin
  ...
  ModifiedMembers := TBoldMemberList.Create; // Now defined before use
  ModifiedMembers.CloneMembers := False;

  SaveOldValues;

  // First pass: Recreate objects and restore attributes/links
  for i := 0 to ObjectIds.Count - 1 do
  begin
    oid := ObjectIds[i];
    RedoValueSpace.EnsureObjectContents(oid);
    ...
    begin
      ...
      else if ObjectAction in [oaNewPersistent, oaTransient] then
      begin
        BoldObject := System.CreateNewObjectByExpressionName(ClassName, ObjectAction = oaNewPersistent);
        System.Locators.UpdateId(BoldObject.BoldObjectLocator, oid, True);
      end;
    end;

    if Assigned(BoldObject) and (ObjectAction <> oaDelete) then
    begin
      UnDoObjectAndAttributes(BoldObject, FSObjectContents);
      if BoldObject.BoldClassTypeInfo.IsLinkClass then
        UnDoInnerLinks(BoldObject, FSObjectContents); // Uses ModifiedMembers
    end;
  end;

  // Second pass: Restore non-inner link relationships
  for i := 0 to ObjectIds.Count - 1 do
  begin
    oid := ObjectIds[i];
    BoldObject := System.Locators.ObjectByID[oid];
    if Assigned(BoldObject) then
      UnDoLinks(BoldObject, UndoValueSpace.GetFSObjectContentsByObjectId(oid)); // Uses ModifiedMembers
  end;
