...
type
  ...
  private
    fData: TBoldBlobData;
    fBlobAttr: TBABinaryBlob;
    fPosition: Int64;
    function GetBlobSize: Int64;
    procedure InternalSetSize(NewSize: Int64; BlobEvents: Boolean);
  public
    constructor Create(BlobAttr: TBABinaryBlob);
    function Read(var Buffer; Count: Longint): Longint; override;
    function Write(const Buffer; Count: Longint): Longint; override;
    function Seek(Offset: Longint; Origin: Word): Longint; override;
    procedure Clear;
    procedure Truncate;
    procedure LoadFromStream(Stream: TStream; AClear: boolean = false);
    procedure LoadFromFile(const aFileName: string; aMode: Word = fmShareDenyNone; AClear: boolean = true);
  ...
  {---TBABinaryBlob---}
  TBABinaryBlob = class(TBoldAttribute)
  strict private
    class var AttributeTypeInfo: TBoldElementTypeInfo;
  private
    class procedure ClearAttributeTypeInfo;
  private
    ...
    function GetAsBytes: TBoldBlobData;
    procedure SetAsBytes(const NewValue: TBoldBlobData);
    function SetContent(const NewValue: TBoldBlobData): boolean;
    procedure SetDataValue(const NewValue: TBoldBlobData);
  protected
    function GetAttributeTypeInfoForType: TBoldElementTypeInfo; override;
    procedure FreeContent; override;
    function GetProxy(Mode: TBoldDomainElementProxyMode): TBoldMember_Proxy; override;
    ...
    function MaySetValue(NewValue: TBoldBlobData; Subscriber: TBoldSubscriber): Boolean; virtual;
  ...
  public
    ...
    procedure AssignValue(const Source: IBoldValue); override;
    procedure SetAsVariant(const Value: Variant); override;
    function CanSetValue(NewValue: TBoldBlobData; Subscriber: TBoldSubscriber): Boolean;
    ...
    property AsBytes: TBoldBlobData read GetAsBytes write SetAsBytes;
  end;

  TBAAnsiBlobEncodingErrorAction = (teaTrim, teaRaise);

  TBAAnsiBlob = class(TBABinaryBlob)
  protected
    function GetAsBlob: TBoldAnsiString;
    procedure SetAsBlob(NewValue: TBoldAnsiString);
    function GetStringRepresentation(Representation: TBoldRepresentation): string; override; // is this needed ?
    procedure SetStringRepresentation(Representation: TBoldRepresentation; const Value: string); override;
  public
    function ValidateString(const Value: string; Representation: TBoldRepresentation): Boolean; override;
  end;

  TBAUnicodeBlob = class(TBABinaryBlob)
  protected
  ...
  {---TBABlob---}
  TBABlob = class({$IFDEF BoldUnicodeBlob}TBAUnicodeBlob{$ELSE}TBAAnsiBlob{$ENDIF})
  end;
...
var BAAnsiBlobEncodingErrorAction: TBAAnsiBlobEncodingErrorAction = teaTrim;
var BoldUnicodeBlobEncoding: TEncoding;
...
begin
  ...
  TBAAnsiBlob.ClearAttributeTypeInfo;
  TBAUnicodeBlob.ClearAttributeTypeInfo;
...
type
  ...
  private
    function GetProxedBlob: TBABinaryBlob;
    function GetAsBytes: TBoldBlobData;
    procedure SetAsBytes(const NewValue: TBoldBlobData);
    function GetBlobAsStream: TStream;
    procedure BeginSupressEvents;
    procedure EndSupressEvents;
    property ProxedBlob: TBABinaryBlob read GetProxedBlob;
  end;
...
constructor TBoldBlobStream.Create(BlobAttr: TBABinaryBlob);
begin
  inherited Create;
  fBlobAttr := BlobAttr;
end;
...
procedure TBoldBlobStream.LoadFromStream(Stream: TStream; AClear: boolean);
var
  Count: Integer;
...
begin
  ...
  try
    ...
    EventsSuppressed := true;
      ...
      fBlobAttr.EndSupressEvents;
      EventsSuppressed := false;
      fBlobAttr.EndModifyOfBlob;
  except
    if not EventsSuppressed then
      fBlobAttr.EndSupressEvents;
    fBlobAttr.FailModifyOfBlob;
    raise;
  end;
...
procedure TBoldBlobStream.LoadFromFile(const aFileName: string; aMode: Word; AClear: boolean);
var
  Stream: TStream;
begin
  Stream := TFileStream.Create(aFileName, aMode);
  try
    LoadFromStream(Stream, AClear);
  finally
    Stream.Free;
  end;
end;
...
procedure TBAAnsiBlob.SetStringRepresentation(Representation: TBoldRepresentation; const Value: string);
begin
  if not ValidateString(Value, Representation) then
    BoldRaiseLastFailure(self, Meth_SetStringRepresentation, sStringValidationFailed);
  case Representation of
    brDefault: SetDataValue(TEncoding.ANSI.GetBytes(Value));
...
function TBAAnsiBlob.ValidateString(const Value: string; Representation: TBoldRepresentation): Boolean;
begin
  result := true;
  if String(AnsiString(Value))<>Value then
  begin
    result := false;
    SetBoldLastFailureReason(TBoldFailureReason.CreateFmt(sStringEncodingError, [Value, 'ANSI'], self));
  end;
end;

function TBABinaryBlob.SupressEvents: Boolean;
begin
  result := fSupressEventCount > 0;
end;

procedure TBABinaryBlob.BeginSupressEvents;
begin
  Inc(fSupressEventCount);
  StartModifyOfBlob('Write');
end;

procedure TBABinaryBlob.EndSupressEvents;
begin
  EndModifyOfBlob;
...
function TBAAnsiBlob.GetStringRepresentation(Representation: TBoldRepresentation): string;
begin
  BoldClearLastFailure;
  if not CanRead(nil) then
    BoldRaiseLastFailure(self, Meth_GetStringRepresentation, '');
...
function TBAAnsiBlob.GetAsBlob: TBoldAnsiString;
begin
  try
    result := TBoldAnsiString(TEncoding.ANSI.GetString(AsBytes));
  except
    on E:EEncodingError do
    case BAAnsiBlobEncodingErrorAction of
      teaTrim: result := AnsiString(TEncoding.Unicode.GetString(AsBytes));
      teaRaise: raise;
    end;
  end;
end;

function TBABinaryBlob.GetAsStream: TBoldBlobStream;
begin
...
function TBABinaryBlob.GetAsBytes: TBoldBlobData;
begin
  result := GetAsStream.fData;
end;

procedure TBABinaryBlob.SetAsBytes(const NewValue: TBoldBlobData);
var
  Count: Integer;
begin
  try
    StartModifyOfBlob('SetAsBytes');
    Count := Length(NewValue);
    AsStream.InternalSetSize(Count, Count = 0);
    if Count <> 0 then
    begin
      GetAsStream.fData := NewValue;
    end;
    EndModifyOfBlob;
  except
    FailModifyOfBlob;
...
function TBABinaryBlob.SetContent(const NewValue: TBoldBlobData): boolean;
begin
  result := false;
  if (BoldPersistenceState = bvpsInvalid) or (AsBytes <> NewValue) then
  begin
    PreChange;
...
function TBABinaryBlob.MaySetValue(NewValue: TBoldBlobData;
  Subscriber: TBoldSubscriber): Boolean;
begin
  Result := True;
end;

function TBABinaryBlob.CanSetValue(NewValue: TBoldBlobData;  Subscriber: TBoldSubscriber): Boolean;
begin
  result := MaySetValue(NewValue, Subscriber);
{$IFNDEF BOLD_NO_QUERIES}
   result := result and SendQuery(bqMaySetValue, [NewValue], Subscriber)
...
procedure TBABinaryBlob.SetDataValue(const NewValue: TBoldBlobData);
var
  bContentIsNull: Boolean;
  sOldValue: TBoldBlobData;
begin
  if (AsBytes <> NewValue) then
  begin
    BoldClearLastFailure;
    if not CanSetValue(NewValue, nil) then
      BoldRaiseLastFailure(self, sSetDataValue, '');
...
function TBABinaryBlob.GetAttributeTypeInfoForType: TBoldElementTypeInfo;
begin
  if not Assigned(AttributeTypeInfo) then
  begin
    AttributeTypeInfo := inherited GetAttributeTypeInfoForType;
...
procedure TBABinaryBlob.AssignContentValue(const Source: IBoldValue);
var
  s: IBoldBlobContent;
  NullableValue: IBoldNullableValue;
begin
  ...
  begin
    ...
    SetContent(s.AsBytes)
  else
    raise EBold.CreateFmt(sUnknownTypeOfSource, [classname, sAssignContentValue]);
end;

function TBABinaryBlob.GetBlobSize: Int64;
begin
  result := AsStream.GetBlobSize;
end;

function TBABinaryBlob.GetFreeStandingClass: TBoldFreeStandingElementClass;
begin
  result := TBFSBlob;
end;

procedure TBAAnsiBlob.SetAsBlob(NewValue: TBoldAnsiString);
begin
  SetStringRepresentation(brDefault, String(NewValue));
end;

procedure TBABinaryBlob.SetAsVariant(const Value: Variant);
var
  p: Pointer;
begin
...
procedure TBABinaryBlob.SetToNull;
begin
  inherited;
  AsStream.Clear;
end;

procedure TBABinaryBlob.StartModifyOfBlob(const Operation: String);
begin
...
procedure TBABinaryBlob.EndModifyOfBlob;
begin
...
procedure TBABinaryBlob.FailModifyOfBlob;
begin
...
procedure TBABinaryBlob.Assign(Source: TBoldElement);
begin
  if (Source is TBABinaryBlob) then
  begin
    var SourceBlob := Source as TBABinaryBlob;
    if SourceBlob.IsNull then
      SetToNull
    else
    begin
      SourceBlob.AsStream.Position:=0;
      LoadFromStream(SourceBlob.AsStream);
...
class procedure TBABinaryBlob.ClearAttributeTypeInfo;
begin
  AttributeTypeInfo := nil;
end;

procedure TBABinaryBlob.AssignValue(const Source: IBoldValue);
var
  s: IBoldBlobContent;
begin
  if source.QueryInterface(IBoldBlobContent, S) = S_OK then
    SetDataValue(s.AsBytes)
  else
    raise EBold.CreateFmt(sUnknownTypeOfSource, [classname, sAssignValue]);
end;
...
function TBABlob_Proxy.GetAsBytes: TBoldBlobData;
begin
  result := ProxedBlob.AsBytes;
end;

procedure TBABlob_Proxy.SetAsBytes(const NewValue: TBoldBlobData);
var
  bContentIsNull: Boolean;
  sOldValue: TBoldBlobData;
  HasSetContent: Boolean;
begin
  ProxedBlob.AsBytes := NewValue;
  sOldValue := ProxedBlob.AsBytes;
  bContentIsNull := ProxedBlob.ContentIsNull;
  sOldValue := ProxedBlob.AsBytes;
  HasSetContent := TBAUnicodeBlob(ProxedBlob).SetContent(NewValue);
  if HasSetContent then
    if bContentIsNull then
...
function TBABinaryBlob.CompareToAs(CompareType: TBoldCompareType;
  BoldElement: TBoldElement): Integer;
var
  CompareBlob: TBABinaryBlob;
begin
  if BoldElement is TBABinaryBlob then
  begin
    CompareBlob := BoldElement as TBABinaryBlob;
...
destructor TBABinaryBlob.Destroy;
begin
  FreeAndNil(fStream);
  inherited;
end;

procedure TBABinaryBlob.Initialize;
begin
  inherited;
end;

function TBABinaryBlob.IsEqualAs(CompareType: TBoldCompareType;
...
function TBABinaryBlob.IsEqualToValue(const Value: IBoldValue): Boolean;
var
...
begin
  ...
  begin
    ...
    if IsNull or vBlob.IsNull then
      ...
      result := Self.AsBytes = vBlob.AsBytes;
  end
  else
    result := inherited IsEqualToValue(Value);
end;

function TBABinaryBlob.ProxyInterface(const IId: TGUID; Mode: TBoldDomainElementProxyMode; out Obj): Boolean;
begin
...
function TBABinaryBlob.GetProxy(Mode: TBoldDomainElementProxyMode): TBoldMember_Proxy;
begin
  result := TBABlob_Proxy.MakeProxy(self, mode);
end;

procedure TBABinaryBlob.FreeContent;
begin
  inherited;
...
procedure TBABinaryBlob.SetEmptyValue;
begin
...
procedure TBABinaryBlob.LoadFromFile(const aFileName: string; aMode: Word; AClear: boolean);
begin
  AsStream.LoadFromFile(aFilename, aMode, AClear);
end;

procedure TBABinaryBlob.LoadFromStream(Stream: TStream; AClear: boolean = false);
begin
  AsStream.LoadFromStream(Stream, AClear);
end;

procedure TBABinaryBlob.SaveToFile(const FileName: string);
begin
  AsStream.SaveToFile(FileName);
end;

procedure TBABinaryBlob.SaveToStream(Stream: TStream);
begin
...
function TBAUnicodeBlob.GetAsBlob: String;
begin
  result := BoldUnicodeBlobEncoding.GetString(AsStream.fData);
end;

procedure TBAUnicodeBlob.SetAsBlob(NewValue: String);
begin
  SetStringRepresentation(brDefault, NewValue);
end;

function TBAUnicodeBlob.GetStringRepresentation(Representation: TBoldRepresentation): string;
begin
  BoldClearLastFailure;
  if not CanRead(nil) then
    BoldRaiseLastFailure(self, Meth_GetStringRepresentation, '');
  case Representation of
    brDefault: begin
      if IsNull then {IsNull ensures current}
        ...
        Result := BoldUnicodeBlobEncoding.GetString(AsBytes);
    end;
    brShort: begin
      Result := '';
    end;
...
procedure TBAUnicodeBlob.SetStringRepresentation(Representation: TBoldRepresentation; const Value: string);
begin
  if not ValidateString(Value, Representation) then
    BoldRaiseLastFailure(self, Meth_SetStringRepresentation, sStringValidationFailed);
  case Representation of
    brDefault: SetDataValue(BoldUnicodeBlobEncoding.GetBytes(Value));
    brShort: {Content type is lost when assigned to a Blob};
  else
    inherited SetStringRepresentation(Representation, Value);
  end;
...
function TBABlob_Proxy.GetProxedBlob: TBABinaryBlob;
begin
  result := ProxedMember as TBABinaryBlob;
end;
...
initialization
  BoldUnicodeBlobEncoding := TEncoding.Unicode;
  ...
  begin
    ...
    AddMemberTypeDescriptor(TBABinaryBlob, alConcrete);
    AddMemberTypeDescriptor(TBABlob, alConcrete);
    AddMemberTypeDescriptor(TBATypedBlob, alConcrete);
    AddMemberTypeDescriptor(TBAAnsiBlob, alConcrete);
    AddMemberTypeDescriptor(TBAUnicodeBlob, alConcrete);
...
finalization
  ...
  if BoldMemberTypesAssigned then
    ...
    begin
      ...
      RemoveDescriptorByClass(TBABinaryBlob);
      RemoveDescriptorByClass(TBABlob);
      RemoveDescriptorByClass(TBATypedBlob);
      RemoveDescriptorByClass(TBAAnsiBlob);
      RemoveDescriptorByClass(TBAUnicodeBlob);
