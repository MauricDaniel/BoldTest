...
type
  ...
  public
    ...
    function Append(const Value: TStringBuilder): TStringBuilder; overload;    
    ...
    function ToString: string; overload; {$IFDEF BOLD_DELPHI16_OR_LATER}override;{$ENDIF}
...
function UIntToStrSB(Value: Cardinal): string; overload;
begin
  FmtStr(Result, '%u', [Value]);
end;

{$IFDEF BOLD_DELPHI13_OR_LATER}
function UInt64ToStr(Value: UInt64): string; overload;
begin
  FmtStr(Result, '%u', [Value]);
end;
{$ENDIF}
...
function TStringBuilder.Append(const Value: TStringBuilder): TStringBuilder;
begin
  if Value.Length > 0 then
  begin
    Length := Length + Value.Length;
    Move(Value.FData[0], FData[Length - Value.Length], Value.Length * SizeOf(Char) );
  end;
  Result := self;
end;
