...
type
  ...
  private
    class var fActiveFormSaver: TBoldFormSaver;
  private
...
begin
  ...
  // At this point events for owning form have been restored from dfm
  // so setting Activate/Deactivate will overwrite them
  // To avoid this store original Activate/Deactivate events and call them from our _Activate/_Deactivate
  if Owner is TForm then begin
    if Assigned(TForm(Owner).OnActivate) then
      raise Exception.Create('Owning form of FormSaver implements OnActivate, not currently supported.');
    if Assigned(TForm(Owner).OnDeactivate) then
      raise Exception.Create('Owning form of FormSaver implements OnDeactivate, not currently supported.');
...
begin
  result := (self = fActiveFormSaver) or (not Assigned(fActiveFormSaver) and Assigned(Form) and Form.Active);
...
begin
  fActiveFormSaver := self;
end;

procedure TBoldFormSaver._DeActivate(Sender: TObject);
begin
  if fActiveFormSaver = self then
    fActiveFormSaver := nil;
end;
