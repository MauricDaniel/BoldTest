...
begin
  ...
  if vIsFormOrDataModule then
    BoldLog.LogFmt(sValidatingHeader, [(Component as TComponent).Name]);
  ...
  begin
    if result then
      BoldLog.LogFmt(sOKCheckMessage, [(Component as TComponent).Name, ValidateCount - vCounter])
    else
      BoldLog.LogFmt(sErrorCheckMessage, [(Component as TComponent).Name, ValidateCount - vCounter]);
  end;
end;
