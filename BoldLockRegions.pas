...
type
  ...
  private
    function GetItems(i: {$IFDEF BOLD_DELPHI29_OR_LATER}NativeInt{$ELSE}Integer{$ENDIF}): TBoldRegion;
    function GetAsString: string;
  public
    procedure AddList(List: TBoldRegionList);
    procedure AddRegionLookup(Regions: TBoldRegionLookup);
    procedure Assign(List: TBoldRegionList);
    property Items[i: {$IFDEF BOLD_DELPHI29_OR_LATER}NativeInt{$ELSE}Integer{$ENDIF}]: TBoldRegion read GetItems; default;
    property AsString: string read GetAsString;
  end;
...
function TBoldRegionList.GetItems(i: {$IFDEF BOLD_DELPHI29_OR_LATER}NativeInt{$ELSE}Integer{$ENDIF}): TBoldRegion;
begin
